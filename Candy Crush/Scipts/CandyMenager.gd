...
func clear_matrix():
	for x in range(9):
		...
		for y in range(12):
			matrix[x].append([])
			matrix[x][y] = null
#Cirando os doces dentro da matrix
func rande_matrix():
	for x in range(9):
		for y in range(12):
			if matrix[x][y] == null:
				matrix[x][y] = gen_candy(x, y)
#gerando os doces para colocar na matrix
func gen_candy(x, y):
	...
	add_child(newCandy)
	return newCandy #retorna um novo doce para dentro da matrix e preenche caso vazio

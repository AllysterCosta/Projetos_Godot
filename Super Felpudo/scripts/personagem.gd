...
func _fixed_process(delta):
	...
	if (walk_right or walk_left) and no_chao:
		sprite.play()
	elif (walk_right or walk_left) and not no_chao:
		sprite.stop()
		sprite.set_frame(3)
	else:
		sprite.stop()
...
func _on_cabea_body_enter( body ):
	if not vivo: return
...
func _on_touchRigth_pressed():
	right = true

func _on_touchRigth_released():
	right = false
